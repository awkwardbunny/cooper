...
int hashTable::findPos(const string &key){
  ...
  if(!data[position].key.compare(key) && !data[position].isDeleted){
    return position;
  }
  ...
  if(position == capacity){             //if the end of the table is reached,
    ...
    //Fixed bug by adding "&& data[position].isOccupied"
  return (!data[position].key.compare(key) && data[position].isOccupied && !data[position].isDeleted) ? position : -1; //if the keys still don't match, entry doesn't exist and return -1
}
...
//Implemented below functions for assignment 2
